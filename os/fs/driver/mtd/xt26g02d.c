...
int xt26g02d_rawread(FAR struct nand_raw_s *raw, off_t block, unsigned int page, FAR void *data, FAR void *spare)
{
...
int xt26g02d_rawwrite(FAR struct nand_raw_s *raw, off_t block, unsigned int page, FAR const void *data, FAR const void *spare)
{
	uint32_t address = (block << 6) + page;	// to get block address, now we populate it
	uint8_t *addr = &address;
